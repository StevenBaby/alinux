...
// 重定位文件读写指针系统调用函数
// 参数 fd 是文件句柄，offset 是新的文件读写指针偏移值
// origin 是偏移的起始位置，是下列三者之一
// SEEK_SET (0，从文件开始处)
// SEEK_CUR (1，从当前读写位置)
// SEEK_END (2，从文件尾处)
int sys_lseek(unsigned int fd, off_t offset, int origin)
{
	...
	// 如果文件句柄值大于程序最多打开文件数 NR_OPEN(20)
	// 或者该句柄的文件结构指针为空，或者对应文件结构的 i 节点字段为空
	// 或者指定设备文件指针是不可定位的，则返回出错码并退出
	if (fd >= NR_OPEN ||
...
int sys_write(unsigned int fd, char *buf, int count)
{
	...
	// 如果文件句柄值大于程序最多打开文件数 NR_OPEN
	// 或者需要写入的字节计数小于0，或者该句柄的文件结构指针为空，则返回出错码并退出
	if (fd >= NR_OPEN || count < 0 || !(file = current->filp[fd]))
		return -EINVAL;

	// 若需读取的字节数 count 等于0，则返回 0，退出
	if (!count)
		return 0;
