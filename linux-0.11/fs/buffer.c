...
// 通过不让中断过程改变缓冲区，而是让调用者来执行，避免了竞争条件（当然除改变数据以外）
// 注意！由于中断可以唤醒一个调用者，因此就需要开关中断指令（cli-sti）序列来检测等待调用返回
// 但需要非常地快(希望是这样)

// 注意！这里有一个程序应不属于这里：
// 检测软盘是否更换
// 但我想这里是放置该程序最好的地方了，因为它需要使已更换软盘缓冲失效
...
// 注意！尽管目前该子程序仅用于软盘
// 以后任何可移动介质的块设备都将使用该程序
// mount/open 操作是不需要知道是否是软盘或其它什么特殊介质的
...
// 由于我们没有对缓冲区上锁（除非我们正在读取它们中的数据）
// 那么当我们（进程）睡眠时缓冲区可能会发生一些问题（例如一个读错误将导致该缓冲区出错）
...
// OK，下面是 getblk 函数，该函数的逻辑并不是很清晰，同样也是因为要考虑竞争条件问题
...
repeat:
	...
	do
	{
		...
		// 则让 bh 指向该 tmp 缓冲区头
		// 如果该 tmp 缓冲区头表明缓冲区既没有修改也没有锁定标志置位
	...
	// 注意！！当进程为了等待该缓冲块而睡眠时
	// 其它进程可能已经将该缓冲块加入进高速缓冲中，所以要对此进行检查
	...
	// 然后根据此新的设备号和块号，重新插入空闲链表和 hash 队列新位置处，并最终返回缓冲头指针
...
// bread_page 一次读四个缓冲块内容读到内存指定的地址
...
struct buffer_head *breada(int dev, int first, ...)
{
	...
	// 注意，下面 ll_rw_block 参数有一个bug
	// 其中的 bh 应该是 tmp，这个 bug 直到在 0.96 版的内核代码中才被纠正过来
...
// 缓冲区初始化函数
// 参数 buffer_end 是指定的缓冲区内存的末端
// 对于系统有 16MB 内存，则缓冲区末端设置为 4MB
...
void buffer_init(long buffer_end)
{
	...
	// 操作的过程是从缓冲区高端开始划分 1K 大小的缓冲块
	// 与此同时在缓冲区低端建立描述该缓冲块的结构 buffer_head
	// 并将这些 buffer_head 组成双向链表
	// h 是指向缓冲头结构的指针，而 h+1 是指向内存地址连续的下一个缓冲头地址
	// 也可以说是指向 h 缓冲头的末端外，
	// 为了保证有足够长度的内存来存储一个缓冲头结构
	// 需要 b 所指向的内存块地址 >= h 缓冲头的末端，也即要 >= h+1
	while ((b -= BLOCK_SIZE) >= ((void *)(h + 1)))
	{
		...
		// 指向具有相同 hash 值的下一个缓冲头
		h->b_next = NULL;

		// 指向具有相同 hash 值的前一个缓冲头
		h->b_prev = NULL;
		...
		// 如果地址 b 递减到等于 1MB，则跳过 384KB
		if (b == (void *)0x100000)
			// 让 b 指向地址 0xA0000(640KB) 处
			b = (void *)0xA0000;
	}
	// 让 h 指向最后一个有效缓冲头
	h--;
	...
	// 链表头的 b_prev_free 指向前一项（即最后一项）
	free_list->b_prev_free = h;

	// h 的下一项指针指向第一项，形成一个环链
