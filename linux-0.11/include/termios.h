...
// 对于修改参数会影响输出的情况，就需要使用这种形式(参见 tcsetattr()，TCSADRAIN 选项)
#define TCSETSW 0x5403

// 在设置 termios 的信息之前，需要先等待输出队列中所有数据处理完
// 并且刷新(清空)输入队列，再设置（参见tcsetattr()，TCSAFLUSH 选项）
#define TCSETSF 0x5404

// 取相应终端 termio 结构中的信息(参见tcgetattr())
#define TCGETA 0x5405

// 设置相应终端 termio 结构中的信息(参见tcsetattr()，TCSANOW 选项)
#define TCSETA 0x5406

// 在设置终端 termio 的信息之前，需要先等待输出队列中所有数据处理完(耗尽)
// 对于修改参数会影响输出的情况，就需要使用这种形式(参见tcsetattr()，TCSADRAIN 选项)
...
// 等待输出队列处理完毕(空)，如果参数值是 0，则发送一个 break（参见tcsendbreak()，tcdrain()）
...
// 如果是1，则重新开启挂起的输出；
// 如果是2，则挂起输入；
// 如果是3，则重新开启挂起的输入（参见tcflow()）
#define TCXONC 0x540A

// 刷新已写输出但还没发送或已收但还没有读数据
// 如果参数是0，则刷新(清空)输入队列；
// 如果是1，则刷新输出队列；
// 如果是2，则刷新输入和输出队列（参见tcflush()）
...
// 对于本地连接的终端或其它设备，软件载波标志是开启的
// 对于使用 modem 线路的终端或设备则是关闭的
// 为了能使用这两个ioctl 调用，tty 线路应该是以 O_NDELAY 方式打开的
// 这样open() 就不会等待载波
...
// 当生成 SIGINT 和 SIGQUIT 信号时不刷新输入输出队列，
// 当生成 SIGSUSP 信号时，刷新输入队列
#define NOFLSH 0000200

// 发送 SIGTTOU 信号到后台进程的进程组
// 该后台进程试图写自己的控制终端
#define TOSTOP 0000400

// 若设置了 ECHO，则除 TAB、NL、START 和 STOP 以外的 ASCII
// 控制信号将被回显成象 ^X 式样，X 值是控制符 +0x40
#define ECHOCTL 0001000

// 若设置了 ICANON 和 IECHO，则字符在擦除时将显示
#define ECHOPRT 0002000

// 若设置了 ICANON，则 KILL 通过擦除行上的所有字符被回显
#define ECHOKE 0004000

// 输出被刷新，通过键入 DISCARD 字符，该标志被翻转
...
// 丢弃 fildes 指定对象所有已写，但还没传送以及所有已收到但还没有读取的数据
