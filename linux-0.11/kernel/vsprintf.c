...
// 字符串头文件 主要定义了一些有关字符串操作的内联函数
#include <string.h>

// 我们使用下面的宏，这样我们就可以不使用 ctype 库了
#define is_digit(c) ((c) >= '0' && (c) <= '9')

// 该函数将字符数字串转换成整数
// 输入是数字串指针的指针，返回是结果数值，另外指针将前移
...
static char *number(char *str, int num, int base, int size, int precision, int type)
{
	...
	// 若带符号，则宽度值减 1
	// 若类型指出是特殊转换，则对于十六进制宽度再减少 2 位(用于0x)
	...
	// 如果数值 num 为0，则临时字符串='0'；否则根据给定的基数将数值 num 转换成字符形式
	if (num == 0)
		tmp[i++] = '0';
	else
		while (num != 0)
...
int vsprintf(char *buf, const char *fmt, va_list args)
{
	...
	repeat:
		...
		// 此时如果标志域表明不是左靠齐，
		// 则该字段前面放入宽度域值 -1 个空格字符，然后再放入参数字符
		// 如果宽度域还大于 0，则表示为左靠齐
		// 则在参数字符后面添加宽度值-1 个空格字符
		case 'c':
			if (!(flags & LEFT))
				while (--field_width > 0)
					*str++ = ' ';
		...
		// 首先取参数字符串的长度，若其超过了精度域值
		// 则扩展精度域=字符串长度，此时如果标志域表明不是左靠齐
		// 则该字段前放入(宽度值-字符串长度) 个空格字符
		// 然后再放入参数字符串。如果宽度域还大于 0
		// 则表示为左靠齐，则在参数字符串后面
		// 添加(宽度值-字符串长度)个空格字符
		case 's':
			s = va_arg(args, char *);
		...
		// 'd', 'i'代表符号整数，因此需要加上带符号标志。'u'代表无符号整数
		case 'd':
		case 'i':
			flags |= SIGN;
		case 'u':
